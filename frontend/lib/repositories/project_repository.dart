...
class ProjectRepository implements IProjectRepository {
  ...
  Future<List<Project>> syncAndGetProjects() async {
    ...
    try {
      LoggingService.logger.info(
        'ProjectRepository: Getting shared preferences...',
      );
      final prefs = await SharedPreferences.getInstance();

      LoggingService.logger.info(
        'ProjectRepository: Loading projects from database...',
      );
      final localProjects = await _database.allProjects;
      LoggingService.logger.info(
        'ProjectRepository: Loaded ${localProjects.length} projects from database',
      );

      // Clear sync token if no local projects (first time or fresh install)
      if (localProjects.isEmpty) {
        LoggingService.logger.info(
          'ProjectRepository: No projects found, clearing sync token...',
        );
      ...
      LoggingService.logger.info(
        'ProjectRepository: Data sync completed successfully',
      );

      // Update database with sync data
      LoggingService.logger.info(
        'ProjectRepository: Upserting ${syncResponse.projects.length} projects...',
      );
      for (var project in syncResponse.projects) {
        LoggingService.logger.fine(
          'ProjectRepository: Upserting project: $project',
        );
        await _database.upsertProject(project);
      }

      LoggingService.logger.info(
        'ProjectRepository: Upserting ${syncResponse.tasks.length} tasks...',
      );
      ...
      LoggingService.logger.info(
        'ProjectRepository: Returning ${syncedProjects.length} synced projects',
      );
  ...
  Future<Project> createProject(String name, String color) async {
    ...
    try {
      ...
      LoggingService.logger.info(
        'ProjectRepository: Project created successfully with ID ${createdProject.id}',
      );

      return createdProject;
    } catch (e) {
      LoggingService.logger.severe(
        'ProjectRepository: Error creating project: $e',
      );
      rethrow;
    }
  }
  ...
  Future<void> updateProject(Project project) async {
    LoggingService.logger.info(
      'ProjectRepository: Updating project ${project.id}...',
    );
    ...
    try {
      ...
      LoggingService.logger.info(
        'ProjectRepository: Project ${project.id} updated successfully',
      );
    } catch (e) {
      LoggingService.logger.severe(
        'ProjectRepository: Error updating project ${project.id}: $e',
      );
      rethrow;
    }
  }
  ...
  Future<void> deleteProject(int id) async {
    ...
    try {
      ...
      LoggingService.logger.info(
        'ProjectRepository: Project $id deleted successfully',
      );
    } catch (e) {
      LoggingService.logger.severe(
        'ProjectRepository: Error deleting project $id: $e',
      );
      rethrow;
    }
  }
  ...
  Future<void> reorderProjects(List<int> projectIds) async {
    ...
    try {
      ...
      LoggingService.logger.info(
        'ProjectRepository: Projects reordered successfully',
      );
    } catch (e) {
      LoggingService.logger.severe(
        'ProjectRepository: Error reordering projects: $e',
      );
      rethrow;
    }
  }
